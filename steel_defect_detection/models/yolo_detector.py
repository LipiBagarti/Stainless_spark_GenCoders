"""
YOLO detector wrapper — converts Ultralytics YOLO output to Prediction schema.

Usage:
    detector = YOLODetector("models/yolo_best.pt")
...
class YOLODetector:
    """Wraps Ultralytics YOLOv8 to produce standardized Prediction objects."""
    ...
    def __init__(self, weights_path: str, device: str = "auto"):
        """
        Args:
            weights_path: Path to trained YOLOv8 weights (.pt file)
        ...
        logger.info(f"YOLO detector loaded from {self.weights_path} on {self.device}")
